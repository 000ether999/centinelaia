...
Nmap done: 1 IP address (1 host up) scanned in 8.42 seconds`;
    ...
    expect(findings.map(({ category, severity }) => ({ category, severity }))).toEqual([
      { category: 'server-fingerprint', severity: 'low' },
      { category: 'server-fingerprint', severity: 'low' },
      { category: 'server-fingerprint', severity: 'info' },
    ]);
    expect(JSON.parse(findings[0]!.rawValue!)).toEqual(rows[0]);
    expect(JSON.parse(findings[2]!.rawValue!)).toEqual(rows[2]);
